...
| P3 HiFT 回归 Student（从 source 学 Teacher） | 08 月初 | ❌ 失败：Student 连单样本都表达不了（单样本 corr≈0.007），全部趋向静态谱 | `docs/PITFALLS_AND_FIXES.md` 第 6 类 |
...
| QAIRT QNN 全图 HTP 迁移（HiFT/Flow） | 08 月 | ⏳ 进行中：converter 语义已证明（P4.2 WAV corr=1.0）；A16W16 快但坏音（corr 0.18）；A16W8 per-channel + conv_post FP16 达 24.77ms 但 PCM corr 0.835，未过最终音频门槛 | — |
